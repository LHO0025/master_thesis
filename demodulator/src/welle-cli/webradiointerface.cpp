        ...
        if (req.is_get) {
            if (req.url == "/") {
                ...
                if (decode_settings.outputCodec == OutputCodec::MP3)
                {
                    ...
                    const std::regex regex_buffered_audio(R"(^/buffered_audio\?sid=([^&]+)&offsetMs=([^&]+)$)");
                    std::smatch match_buffered_audio;
                    if (regex_search(req.url, match_buffered_audio, regex_buffered_audio)) {
                        success = send_buffered_stream(s, match_buffered_audio[1], match_buffered_audio[2]);
                    }
...
bool WebRadioInterface::send_mux_json(Socket& s)
{
    ...
    try {
        ...
        std::cerr << "Exception during mux JSON serialization: " << e.what() << std::endl;
        return false;
    }
}
    ...
    try {
        for (const auto& srv : rx->getServiceList()) {
            if (rx->serviceHasAudioComponent(srv) &&
                (to_hex(srv.serviceId, 4) == stream ||
                 (uint32_t)std::stoul(stream) == srv.serviceId)) {

                auto& ph = phs.at(srv.serviceId);

                double offsetSeconds = std::stod(offsetMsStr) / 1000;
                double index = offsetSeconds * static_cast<double>(ph.rate * 2);
                long long preciseIndex = std::round(index);

                if (preciseIndex > static_cast<long long>(ph.audioBuffer.size())) {
                ...
                if (s.send(response_headers.data(), response_headers.size(), 0) < 0) {
                    std::cerr << "Failed to send HTTP headers" << std::endl;
                    return false;
                }


                const int sampleRate = ph.rate;
                const int numChannels = 2;
                const int bitsPerSample = 16;
                const int byteRate = sampleRate * numChannels * bitsPerSample / 8;
                const int blockAlign = numChannels * bitsPerSample / 8;

                const uint32_t dataLength = 0xFFFFFFF0;
                const uint32_t totalLength = 36 + dataLength;

                std::ostringstream wav;
                wav.write("RIFF", 4);
                wav.write(reinterpret_cast<const char*>(&totalLength), 4);
                wav.write("WAVE", 4);
                wav.write("fmt ", 4);
                ...
                wav.write("data", 4);
                wav.write(reinterpret_cast<const char*>(&dataLength), 4);

                std::string wavHeader = wav.str();

                if (s.send(wavHeader.data(), wavHeader.size(), 0) < 0) {
                    std::cerr << "Failed to send WAV header" << std::endl;
                    return false;
                }

                ProgrammeSender sender(std::move(s));
                sender.isLive = false;
