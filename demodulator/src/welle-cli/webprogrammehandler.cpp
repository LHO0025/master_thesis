...
bool ProgrammeSender::send_cached_stream(WebProgrammeHandler& webProgrammeHandler, ssize_t index) {
    ...
    while (running) {
        ...
        std::vector<int16_t> samples = webProgrammeHandler.getAudioBufferChunk(cached_mp3_index);
        if (static_cast<int>(samples.size()) == AUDIO_CHUNK_SIZE) {
            cached_mp3_index += AUDIO_CHUNK_SIZE;
            ret = s.send(samples.data(), samples.size() * sizeof(int16_t), flags);
        } 
...
void WebProgrammeHandler::cache_mp3(const std::vector<int16_t>& data) {
...
std::vector<int16_t> WebProgrammeHandler::getAudioBufferChunk(int index)
{
    std::unique_lock<std::mutex> mp3_lock(cached_mp3_mutex);
